  ...
  <el-row :gutter="20">
    <el-col :sm="12">
      <el-card shadow="hover">
        <el-descriptions :title="t('dashboardAdmin.serverInfo')" :column="4" direction="vertical">
          ...
          <el-descriptions-item
            :label="t('dashboardAdmin.vendor')"
            v-if="sysinfo.product_name !== ''"
            >{{ sysinfo.product_name }}</el-descriptions-item
          >
          <el-descriptions-item label="CPU"
            >{{ sysinfo.physical_core_count }} cores / {{ sysinfo.arch }}</el-descriptions-item
          >
          <el-descriptions-item label="Memory">{{ sysinfo.memory_total }}</el-descriptions-item>
          <el-descriptions-item :label="t('dashboardAdmin.bootTime')">{{
            sysinfo.boot_time
          }}</el-descriptions-item>
        ...
        <el-row :gutter="24" style="text-align: center">
          <el-col
            ...
            style="
              ...
              font-weight: bold;
            "
            >{{ t('dashboardAdmin.serverStatus') }}</el-col
          >
          <el-col :sm="6">
            <el-progress type="dashboard" :percentage="load_avg"></el-progress>
            <div>{{ t('dashboardAdmin.systemLoad') }}</div>
          </el-col>
          <el-col :sm="6">
          ...
          <el-col :sm="6">
            ...
            <div>{{ t('dashboardAdmin.memory') }}</div>
          </el-col>
          <el-col :sm="6">
            <el-progress type="dashboard" :percentage="disk_root_avg" />
            <div>{{ t('dashboardAdmin.diskRoot') }}</div>
          </el-col>
        </el-row>
      </el-card>
    </el-col>
    ...
    <el-row :gutter="24">
      ...
      <el-col :sm="12">
        <el-card>
          <template #header>
            <div class="card-header">
              <span>{{ t('dashboardAdmin.memory') }}</span>
            </div>
          </template>
      ...
      <el-col :sm="12">
        <el-card>
          <template #header>
            <div class="card-header">
              <span>{{ t('dashboardAdmin.systemLoad') }}</span>
            </div>
          </template>
...
onMounted(async () => {
  ...
  memory_chart = new Chart(memory_container, {
    type: 'line',
    data: {
      labels: [],
      datasets: [
  ...
  loadavg_chart = new Chart(loadavg_container, {
    type: 'line',
    data: {
      labels: [],
      datasets: [
  ...
  network_chart = new Chart(network_container, {
    type: 'line',
    data: {
      labels: [],
      datasets: [
...
const loadSystemInfo = async () => {
  ...
  let memory_prc = (
    ((sysinfo.value.memory_total_b - sysinfo.value.available_memory_b) /
      sysinfo.value.memory_total_b) *
    100
  ).toFixed(2)
  memeory_avg.value = parseFloat(memory_prc)
  if (isArray(sysinfo.value.disk_info)) {
    sysinfo.value.disk_info.forEach(
      (item: { mount_point: string; available_space: number; total_space: number }) => {
        if (item.mount_point === '/') {
          disk_root_avg.value = parseFloat(
            ((item.available_space / item.total_space) * 100).toFixed(2),
          )
        }
      },
    )
...
const FetchRTStatus = async () => {
  ...
  if (resp.code === 0) {
    ...
    load_avg.value = parseFloat(
      ((sysinfo.value.loadavg_one / sysinfo.value.cpu_num) * 100).toFixed(2),
    )
    cpu_avg.value = parseFloat(sysinfo.value.cpu_usage.toFixed(2))
    let memory_prc = (
      ((sysinfo.value.memory_total_b - sysinfo.value.available_memory_b) /
    ...
    resp.data.system_stats.forEach((element: Record<string, any>) => {
      ...
      lables.push(
        tm.getHours().toString() +
          ':' +
          tm.getMinutes().toString() +
          ':' +
          tm.getSeconds().toString(),
      )
    })
