  ...
  <div class="entities-container">
    <el-card shadow="never">
      <template #header>
        <div class="card-header">
          <div>
            <span>同步运行环境</span>
            <span class="card-sub">独立系统用户模式：为存量用户补齐 Linux 账号与家目录赋权</span>
          </div>
      ...
      <el-alert
        title="系统固定使用「独立系统用户」模式（已移除统一 www 模式）：每个面板用户对应一个 Linux 账号（nologin），网站与 PHP-FPM 均以该账号运行。新建用户时系统已自动补齐运行环境，本页用于存量用户一键修复补齐（幂等操作、不影响已有站点）。"
        type="info"
        :closable="false"
        show-icon
      ...
      <div class="mode-cards">
        <div class="mode-card active">
          <div class="mode-card-head">
            ...
            <el-tag type="warning" size="small" effect="dark">当前模式</el-tag>
          </div>
    ...
    <el-dialog v-model="resultVisible" title="同步结果" width="760px" top="8vh">
      <div class="result-summary">
        ...
        <span v-if="resultMode" class="result-mode">按「独立系统用户」模式执行</span>
      </div>
